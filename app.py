...
def ats_score(text: str, skills: list) -> dict:
    """Heuristic ATS score (1-10) + breakdown. More honest for junior profiles."""
    score = 5.0
    reasons = []
    text_lower = text.lower()
    ...
    if verb_count >= 5:
        ...
        reasons.append("⚠️ Use more action verbs (led, developed, implemented, diagnosed...)")
    ...
    if intern_count >= 2:
        reasons.append("⚠️ Multiple internship roles only – highlight projects, certifications and measurable impact more strongly")
        score -= 0.3
    if "graduate" in text_lower or "bachelor" in text_lower:
        reasons.append("ℹ️ Recent graduate profile – target Junior / Entry-level / Graduate roles, not Senior titles")

    if re.search(r"\b(senior|lead|principal|manager)\b", text_lower[:400]):
        if intern_count >= 1 or "graduate" in text_lower:
            reasons.append("❌ Header claims advanced titles while experience is mostly internships – this can hurt credibility. Use Junior / ICT Support Specialist instead")
            score -= 0.8
...
if not st.session_state.cv_text:
    ...
    with tab6:
        ...
        default_query = suggestions[0] if suggestions else "IT Support Specialist"
        q = st.text_input("Search query", value=default_query, key="search_q")
        loc = st.text_input("Location preference", value="Kenya OR remote", key="search_loc")
