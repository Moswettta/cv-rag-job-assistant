...
def extract_skills(text: str) -> list:
    ...
    common_skills = [
        ...
        "end-user", "incident", "troubleshooting", "mysql", "php", "html5", "css3",
    ]
...
def extract_experience_years(text: str) -> float:
    """Estimate years of *work* experience by summing real date ranges.
    Ignores education-only year ranges and crude marker counting.
    """
    patterns = [
...
def ats_score(text: str, skills: list) -> dict:
    """
    ATS-friendly scoring based on industry best practices:
    - Simple single-column layout (no tables/graphics/text boxes)
    - Standard headings (Work Experience, Education, Skills...)
    - Relevant keywords
    - Clean, readable content
    - Quantifiable results & action verbs
    """
    score = 5.0
    reasons = []
    text_lower = text.lower()
    ...
    action_verbs = [
        ...
        "resolved", "maintained", "trained", "documented"
    ]
    ...
    if verb_count >= 6:
        ...
        reasons.append("⚠️ Use more action verbs (diagnosed, configured, resolved, improved...)")
    ...
    if re.search(r"\d+%|\$\d+|\d+\s*(?:users|clients|projects|team|tickets|issues)", text, re.I):
        score += 0.8
        reasons.append("✅ Quantifiable results present (numbers help ranking and human readers)")
    else:
        reasons.append("⚠️ Add measurable impact (e.g. “Reduced resolution time by 40%”, “Supported 50+ users”)")

    intern_count = len(re.findall(r"\bintern\b", text_lower))
    if intern_count >= 2:
        reasons.append("⚠️ Multiple internship roles only – strengthen Projects, Certifications and the 40% achievement")
        score -= 0.2
    if "graduate" in text_lower or "bachelor" in text_lower:
        reasons.append("ℹ️ Recent graduate profile – target Junior / Entry-level / Graduate roles")

    if re.search(r"\b(senior|lead|principal|manager|engineer)\b", text_lower[:450]):
        if intern_count >= 1 or "graduate" in text_lower:
            reasons.append("❌ Header uses advanced titles while experience is mostly internships. Change to “Junior IT Support Specialist” or “ICT Support Officer” for credibility and ATS honesty")
            score -= 0.9
...
def generate_linkedin_about(cv_text: str, skills: list, years: float) -> str:
    """Generate a LinkedIn About / Summary section (realistic for experience level)."""
    skill_str = ", ".join(skills[:10]) if skills else "technical support, networking and databases"
    
    if years >= 5:
        level_phrase = f"Experienced professional with {years:.0f}+ years"
    elif years >= 1.5:
        level_phrase = f"IT professional with {years:.1f} years of hands-on experience"
    elif years >= 0.5:
        level_phrase = f"Motivated IT Support professional with practical internship experience"
    else:
        level_phrase = "Motivated recent graduate and IT enthusiast"

    sentences = [s.strip() for s in re.split(r"[.!?]\s+", cv_text) if 40 < len(s.strip()) < 180]
    highlights = " ".join(sentences[:2]) if sentences else "I focus on delivering reliable technical support and continuous learning."

    about = f"""{level_phrase} specializing in {skill_str}.

{highlights}

I am passionate about solving technical problems, supporting end-users, and continuously expanding my skills in systems administration, databases and networking. Always open to Junior / Entry-level opportunities where I can contribute and grow.
...
def suggest_job_titles(skills: list, years: float) -> list:
    ...
    if has("windows", "linux", "networking", "tcp/ip", "dns", "dhcp") or has("customer service", "end-user"):
        suggestions.extend([
            f"{level}IT Support Specialist",
            f"{level}Technical Support Engineer",
            f"{level}ICT Support Officer",
            "Help Desk Analyst",
    ...
    if has("sql", "mysql", "database"):
        suggestions.extend([
            f"{level}Database Support Specialist",
            "Junior Database Administrator",
            "SQL Support Analyst",
        ])
    if has("oracle"):
        suggestions.append("Oracle Support Engineer (Junior)")
    if has("php", "javascript", "html", "css", "mysql"):
        suggestions.extend([
            f"{level}Web Developer",
    ...
    if has("vmware", "virtualbox", "linux"):
        suggestions.append("Junior Systems Administrator")
...
if not st.session_state.cv_text:
    ...
    with tab1:
        ...
        with col3:
            st.metric("Est. Experience", f"{years:.1f} years" if years else "Not clear")
    ...
    with tab2:
        ...
        job_title = st.text_input("Job Title", placeholder="e.g. Junior IT Support Specialist", key="match_title")
    ...
    with tab6:
        ...
        st.caption("Automated scraping is often blocked. Use the ready-made links below – they open real job search results.")
        default_query = suggestions[0] if suggestions else "Junior IT Support Specialist"
        ...
        q_li = q.replace(" ", "%20")
        st.markdown(f"""
**Direct job search links (recommended):**
...
""")
        ...
        if st.button("🔎 Try automated search (best-effort)"):
            ...
            if jobs:
                ...
                st.warning(
                    "Automated search returned nothing (common). "
                    "Please use the Google / LinkedIn / Indeed / BrighterMonday links above."
                )
